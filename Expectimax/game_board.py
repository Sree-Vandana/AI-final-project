...
@jit
def get_available_from_zeros(a):  # all available zero tiles
...
class GameBoard:
    ...
    def get_available_cells(self):  # Array of cells available
        cells = []
        for x in range(4):
            for y in range(4):
                if self.grid[x][y] == 0:
    ...
    def move(self, dir, get_avail_call=False):  # get all possible moves
        if get_avail_call:
            clone = self.clone()
    ...
    def get_available_moves(self, dirs=dirs):  # what moves are available to move
        available_moves = []
    ...
    def get_cell_value(self, pos):  # get grid cell value
